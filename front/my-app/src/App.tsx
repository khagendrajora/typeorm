...
function App() {
  ...
  const restoreCheckpoints = (checkpoints: any[]) => {
    ...
    // Duplicates have already been filtered before creating markers
    markersRef.current = [...markersRef.current, ...restoredMarkers];
    setCheckPoints((prev) => {
      console.log(`Restoring ${restoredCheckpoints.length} new checkpoints (duplicates were already filtered)`);
      return [...prev, ...restoredCheckpoints];
    });
